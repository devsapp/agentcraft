...
@router.put('/{document_id}', response_model=BasicResponse)
async def update_document(document_id: int, req: UpsertDocumentRequest, token: JWTData = Depends(validate_token)):
    """更新问题"""
    service.update_document(id=document_id, user_id=token.user_id, **vars(req))
    return {"code": 200, "msg": "success"}
