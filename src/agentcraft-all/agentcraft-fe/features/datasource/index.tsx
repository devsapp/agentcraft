...
function DocumentForm({
  ...
  form: UseFormReturnType<DocumentRequestPayload>;
}) {
  return (
    <Box mx="auto">
      <TextInput
...
function QuestionForm({
  ...
  form: UseFormReturnType<QuestionRequestPayload>;
}) {
  return (
    <Box mx="auto">
      <TextInput
...
function AddOrUpdate() {
  ...
  const currentForm: UseFormReturnType<any> =
    dataSetType == DataSetType.DOCUMENT ? documentForm : questionForm;
  useEffect(() => {
    if (isEdit) {
  ...
  return (
    ...
    >
      {dataSetType == DataSetType.DOCUMENT ? (
        <DocumentForm form={documentForm} />
      ) : (
                ...
                const results = await Promise.all(
                  rows.map((row) => {
                    return new Promise(async (resolve) => {
                      /* 此处指定了excel中的列头，可按需修改调整标题和内容所需要的列 */
                      const target =
                        dataSetType === DataSetType.QUESTION
                          ? {
                              url: "",
