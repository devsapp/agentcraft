...
def update_agent(
    ...
    with Session(postgresql.postgres) as session:
        ...
        if exact_datasets:
            _bulk_insert_dataset(session, agent_id, exact_datasets, DatasetType.EXACT_SEARCH)
        if fuzzy_datasets:
            _bulk_insert_dataset(session, agent_id, fuzzy_datasets, DatasetType.FUZZY_SEARCH)
        session.commit()
